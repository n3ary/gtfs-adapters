 ...
 *
 * Moved from n3ary/gtfs's `packages/gtfs-static/src/lib/route-colors.ts`
 * as part of the static-pipeline extraction (gtfs-adapters#1 /
 * n3ary/gtfs#67). The algorithm itself is generic — every feed's
 * routes.txt benefits from the placeholder / OKLCh modal-collision
 * fixup — but the file lives in the cluj adapter for now since cluj
 * is the only feed we ship today. Other feeds can `import` from
 * `@n3ary/gtfs-adapter-cluj-napoca/static` until a shared
 * `gtfs-adapters/shared/` module is carved out (out of scope for
 * this extraction).
 *
 *
